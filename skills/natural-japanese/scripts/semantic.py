...
SEMANTIC_REPETITION_MAX_THRESHOLD = (
    0.9322158694267273  # value<=th → FP率4.9%(n=81), AI検出率46.4%(n=405)
)
TOPIC_JUMP_MIN_THRESHOLD = (
    0.765757143497467  # value>=th → FP率4.9%(n=81), AI検出率62.2%(n=405)
)
...
def print_human_report(path: Path, findings: list[Finding], stats: dict) -> None:
    ...
    print(
        f"文数: {stats['n_sentences']}  モデル: {stats['model']}  genre: {stats['genre'] or '(未指定)'}"
    )
    if stats["skipped"]:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=f"埋め込みモデル名（既定: {DEFAULT_MODEL}）",
    )
