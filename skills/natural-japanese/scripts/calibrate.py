...
REPO_ROOT = SCRIPT_DIR.parents[
    2
]  # scripts/ -> natural-japanese/ -> skills/ -> repo root
...
def run_full_lint(mod, prepared: PreparedDoc) -> None:
    ...
    findings += mod.detect_antithesis_repetition(
        prepared.lines, prepared.raw_lines_by_no
    )
    findings += mod.detect_low_sentence_length_variance(prepared.sentences)
    findings += mod.detect_english_syntax_smell(
        prepared.lines, prepared.raw_lines_by_no
    )

    nominal_and_conj_findings, morph_stats = (
        mod.detect_nominal_ending_and_paragraph_conjunctions(
    ...
    low_spec_findings, low_spec_stats = mod.detect_low_specificity(
        prepared.lines, prepared.raw_lines_by_no
    )
...
def cmd_report(mod) -> None:
    ...
    lines_out.append("")
    lines_out.append(
        "| 検出器 | human_aozora | human_web | human_business | ai | ai_business |"
    )
    lines_out.append("| --- | --- | --- | --- | --- | --- |")
    for category in ALL_CATEGORIES:
        row = [category]
        for corpus_type in (
            ...
            if cell["n_docs"] == 0:
                ...
                row.append(
                    f"{cell['doc_fire_rate']:.0%} / {cell['per_1000_chars']:.2f}"
                )
...
def build_sweep_registry(mod) -> dict[str, DetectorSweepSpec]:
    def run_low_burstiness(p: PreparedDoc, value: float) -> bool:
        findings, _ = mod.detect_rhythm_statistics(
            p.tokenized, burstiness_threshold=value
        )
        return _fired(findings, "low_burstiness")

    def run_high_length_autocorrelation(p: PreparedDoc, value: float) -> bool:
        findings, _ = mod.detect_rhythm_statistics(
            p.tokenized, autocorr_threshold=value
        )
    ...
    def run_antithesis_repetition(p: PreparedDoc, value: float) -> bool:
        findings = mod.detect_antithesis_repetition(
            p.lines, p.raw_lines_by_no, threshold=int(value)
        )
    ...
    def run_repeated_sentence_lead(p: PreparedDoc, value: float) -> bool:
        findings, _ = mod.detect_ngram_repetition(
            p.tokenized, lead_repeat_threshold=int(value)
        )
        return _fired(findings, "repeated_sentence_lead")

    def run_repeated_syntax_template(p: PreparedDoc, value: float) -> bool:
        findings, _ = mod.detect_ngram_repetition(
            p.tokenized, template_ratio_threshold=value
        )
    ...
    def run_low_specificity(p: PreparedDoc, value: float) -> bool:
        findings, _ = mod.detect_low_specificity(
            p.lines, p.raw_lines_by_no, score_threshold=value
        )
    ...
    return {
        ...
        "high_length_autocorrelation": DetectorSweepSpec(
            "high_length_autocorrelation",
            "autocorr_threshold",
            mod.AUTOCORR_THRESHOLD,
            frange(0.1, 0.95, 0.05),
            run_high_length_autocorrelation,
        ),
        "low_sentence_variance": DetectorSweepSpec(
            "low_sentence_variance",
            "threshold",
            mod.SENTENCE_VARIANCE_CV_THRESHOLD,
            frange(0.05, 0.6, 0.02),
            run_low_sentence_variance,
        ),
        "nominal_ending": DetectorSweepSpec(
            "nominal_ending",
            "nominal_ratio_threshold",
            mod.NOMINAL_ENDING_RATIO_THRESHOLD,
            frange(0.05, 0.6, 0.02),
            run_nominal_ending,
        ),
        "paragraph_lead_conjunction": DetectorSweepSpec(
            "paragraph_lead_conjunction",
            "conj_ratio_threshold",
            mod.PARAGRAPH_CONJ_RATIO_THRESHOLD,
            frange(0.05, 0.7, 0.02),
            run_paragraph_lead_conjunction,
        ),
        "uniform_paragraph_structure": DetectorSweepSpec(
            "uniform_paragraph_structure",
            "uniform_cv_threshold",
            mod.UNIFORM_PARAGRAPH_CV_THRESHOLD,
            frange(0.02, 0.5, 0.02),
            run_uniform_paragraph_structure,
        ),
        "antithesis_repetition": DetectorSweepSpec(
            "antithesis_repetition",
            "threshold",
            mod.ANTITHESIS_REPETITION_THRESHOLD,
            [1, 2, 3, 4, 5, 6, 7, 8],
            run_antithesis_repetition,
        ),
        # nested_attributive は 2026-07 コーパス校正で削除済み（弁別力なし、sweep登録も削除）。
        "repeated_sentence_lead": DetectorSweepSpec(
            "repeated_sentence_lead",
            "lead_repeat_threshold",
            mod.NGRAM_LEAD_REPEAT_THRESHOLD,
            [1, 2, 3, 4, 5, 6, 7, 8],
            run_repeated_sentence_lead,
        ),
        "repeated_syntax_template": DetectorSweepSpec(
            "repeated_syntax_template",
            "template_ratio_threshold",
            mod.NGRAM_TEMPLATE_RATIO_THRESHOLD,
            frange(0.1, 0.9, 0.05),
            run_repeated_syntax_template,
        ),
        ...
        "low_lexical_diversity_mtld": DetectorSweepSpec(
            ...
            mod.MTLD_THRESHOLD,
            frange(10, 90, 2),
            run_low_lexical_diversity_mtld,
        ),
        "low_specificity": DetectorSweepSpec(
            "low_specificity",
            "score_threshold",
            mod.LOW_SPECIFICITY_SCORE_THRESHOLD,
            frange(-0.3, 0.6, 0.02),
            run_low_specificity,
        ),
    }
...
def cmd_sweep(mod, detector_name: str) -> None:
    ...
    for value in spec.values:
        ...
        for p in human_prepared:
            try:
                ...
                print(
                    f"Warning: spec.run failed for human_prepared {p}: {e}",
                    file=sys.stderr,
                )
        ai_hit = 0
        for p in ai_prepared:
            try:
                ...
                print(
                    f"Warning: spec.run failed for ai_prepared {p}: {e}",
                    file=sys.stderr,
                )
    ...
    candidates = [
        c for c in curve if c["human_fp_rate"] is not None and c["human_fp_rate"] < 0.05
    ]
    recommended = None
    if candidates:
    ...
    lines_out.append("")
    lines_out.append(
        f"パラメータ: `{spec.param_name}`（現行デフォルト値: {spec.default}）"
    )
...
def cmd_length_analysis(mod) -> None:
    ...
    for label, _, _ in LENGTH_BINS:
        result[label] = {
            cat: {"human_fired": 0, "human_total": 0, "ai_fired": 0, "ai_total": 0}
    ...
    for cat in STATISTICAL_CATEGORIES:
        ...
        for label, lo, _ in LENGTH_BINS:
            ...
            if cell["human_total"] == 0 and cell["ai_total"] == 0:
                continue
            human_rate = (
                cell["human_fired"] / cell["human_total"]
                if cell["human_total"]
                else None
            )
    ...
    lines_out = []
    lines_out.append(
        "# calibrate.py length-analysis — 統計指標系検出器の文書長別弁別力"
    )
    lines_out.append("")
    lines_out.append(f"標本数: human={len(human_prepared)}件, ai={len(ai_prepared)}件")
    lines_out.append("")
...
def main() -> int:
    ...
    sweep_parser.add_argument(
        "--detector", required=True, help="検出器名（例: low_burstiness）"
    )
